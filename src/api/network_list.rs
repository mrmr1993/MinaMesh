...
impl MinaMesh {
  pub async fn network_list(&self) -> Result<NetworkListResponse, MinaMeshError> {
    // Trustless mode: report the configured network id (no daemon). Otherwise ask the node
    // (daemon) for its network id, through the trait.
    let network_id = if self.indexer.is_some() { self.network_id.clone() } else { self.node.network_id().await? };
