...
impl MinaMesh {
  pub async fn network_list(&self) -> Result<NetworkListResponse, MinaMeshError> {
    // Trustless mode: report the configured network id (no daemon).
    let network_id = if self.indexer.is_some() {
      self.network_id.clone()
    } else {
      self.graphql_client.send(QueryNetworkId::build(())).await?.network_id
    };
